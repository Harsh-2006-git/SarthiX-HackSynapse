...
const FamilyMode = () => {
  ...
  const watchRef = useRef(null);          // pilgrim GPS watch
  const guardianWatchRef = useRef(null);  // guardian continuous GPS watch
  ...
  /* ─── normalizeCoord: convert any coord format → [lat, lng] ─── */
  ...
  /* ─── addMarker: place / update a map marker ─── */
  const addMarker = (posInput, color, emoji, type) => {
    ...
    if (markerRef && markerRef.current) { markerRef.current.remove(); markerRef.current = null; }
    ...
    if (type === 'live') {
      ...
      el.innerHTML = `
        ...
        <div class="relative w-10 h-10 rounded-full border-2 border-white flex items-center justify-center text-white shadow-2xl" style="background-color:${color};box-shadow:0 4px 20px ${color}99;font-size:17px;">
          ${emoji}
        </div>
      `;
      ...
      el.className = 'relative flex items-center justify-center';
      el.innerHTML = `
        <div style="position:absolute;width:44px;height:44px;border-radius:50%;background:rgba(37,99,235,0.2);animation:pulse 2s infinite;top:-7px;left:-7px;pointer-events:none;"></div>
        <div style="width:30px;height:30px;border-radius:50%;background:${color};border:2px solid white;display:flex;align-items:center;justify-content:center;font-size:15px;box-shadow:0 4px 15px ${color}80;">🛡️</div>
      `;
    } else {
      el.className = 'w-9 h-9 rounded-full border-2 border-white flex items-center justify-center text-white shadow-lg';
  ...
  const stopGuardianTracking = useCallback(() => {
    if (guardianWatchRef.current) {
      navigator.geolocation.clearWatch(guardianWatchRef.current);
      guardianWatchRef.current = null;
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      if (role === 'pilgrim') {
        ...
        // ─── Start CONTINUOUS guardian GPS (not just one-shot) ───
  ...
  useEffect(() => {
    ...
    // Filter: if we're watching someone else, skip
    if (selectedProtégé && Number(selectedProtégé) !== uid) return;

    // Auto-select if no one is selected yet
    if (!selectedProtégé) setSelectedProtégé(uid);

    const lat = parseFloat(lastLocation.lat);
    const lng = parseFloat(lastLocation.lng);
    ...
    // Sync trip stats sent by traveler
      ...
      fetch(`https://router.project-osrm.org/route/v1/driving/${lng},${lat};${dest.lng},${dest.lat}?overview=false`)
        ...
        .then(data => {
          if (data.routes?.[0]) {
            ...
            setTripStats({ distance: s.distance, eta: Math.ceil(s.duration / 60), speed: lastLocation.speed || 0 });
          }
        }).catch(() => {});
    }
    ...
    // Deduplicate: only append if position changed
    const trail = trailPointsRef.current;
    const lastPt = trail[trail.length - 1];
    if (!lastPt || lastPt[0] !== lat || lastPt[1] !== lng) {
      trailPointsRef.current = [...trail, pos];
    }
    drawTrail(trailPointsRef.current);

    addMarker(pos, '#f43f5e', '📍', 'live');
    mapInstance.current.easeTo({ center: [lng, lat] });
    setStatus(`🟢 LIVE — Signal at ${new Date().toLocaleTimeString()}`);
  ...
  useEffect(() => {
    ...
    if (relevant && !sosActive) {
      ...
      // Use ref for freshest guardian pos (avoids stale closure)
      const gPos = guardianPosRef.current || guardianPos;
      if (mapInstance.current && gPos) {
        drawRoute(gPos, ppos, '#ef4444', 8, true, 'rescue');
      }
    }
  }, [sosAlerts, selectedProtégé, role, sosActive]);
  ...
  /* ─── drawRoute: fetch OSRM route and draw on map ─── */
                    ...
                    {protégés.length === 0 ? (
                      ...
                      <button key={p.mapping_id}
                        onClick={() => {
                          ...
                          if (mapInstance.current) {
                            // Remove trail layers (both glow + solid)
                            if (mapInstance.current.getLayer('trail-glow-layer')) mapInstance.current.removeLayer('trail-glow-layer');
