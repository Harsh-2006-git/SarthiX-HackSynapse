...
const AdminPage = () => {
    ...
    useEffect(() => {
        ...
        if (requestedTab && ['overview', 'users', 'bookings', 'density', 'alerts', 'sos', 'crime'].includes(requestedTab)) {
    ...
    const fetchDashboardData = async () => {
        ...
        try {
            ...
            const crimeRes = await fetch(`${publicApiV1}/crime-heatmap`, { headers });
            if (crimeRes.ok) setCrimeZones((await crimeRes.json()).zones || []);
    ...
    const openCrimeReport = async (zoneId) => {
        const token = localStorage.getItem('token');
        const publicApiV1 = BACKEND_URL.replace('/admin', '/zone');
        const response = await fetch(`${publicApiV1}/crime-report/${zoneId}`, { headers: { Authorization: `Bearer ${token}` } });
        if (response.ok) setCrimeReport(await response.json());
    };
    ...
    const tabs = [
        ...
        { id: 'crime', label: 'Crime Risk', icon: ShieldAlert },
                        ...
                        {activeTab === 'crime' && (
                            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                <div className="rounded-[2rem] bg-slate-950 p-7 text-white shadow-xl">
                                    <div className="flex items-start gap-4"><ShieldAlert className="mt-1 text-rose-400" size={28} /><div><h3 className="text-2xl font-black">Crime risk heatmap</h3><p className="mt-2 max-w-3xl text-sm text-slate-300">Modelled planning estimate from Ujjain district totals. It is not observed crime by temple zone or a police crime rate. Click a zone for its category report.</p></div></div>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                                    {crimeZones.map((zone) => {
                                        const color = zone.live_level === 'HIGH' ? 'border-rose-400 bg-rose-50' : zone.live_level === 'MEDIUM' ? 'border-amber-400 bg-amber-50' : 'border-emerald-400 bg-emerald-50';
                                        return <button key={zone.zone_id} onClick={() => openCrimeReport(zone.zone_id)} className={`text-left rounded-3xl border-2 p-6 transition hover:-translate-y-1 hover:shadow-lg ${color}`}><div className="flex items-center justify-between gap-3"><h4 className="font-black text-slate-900">{zone.zone_name}</h4><span className="rounded-full bg-slate-900 px-3 py-1 text-[10px] font-black text-white">{zone.live_level}</span></div><p className="mt-5 text-4xl font-black text-slate-900">{zone.risk_score}<span className="text-base text-slate-500">/100</span></p><p className="text-xs font-bold text-slate-500">Estimated count: {zone.estimated_total} · Live visitors: {zone.current_count}</p><div className="mt-4 h-2 overflow-hidden rounded-full bg-white"><div className="h-full bg-slate-900" style={{ width: `${zone.risk_score}%` }} /></div></button>;
                                    })}
                                </div>
                                {crimeReport && <div className="rounded-[2rem] bg-white p-7 shadow-sm border border-slate-100"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-2xl font-black text-slate-900">{crimeReport.zone_name} status report</h3><p className="mt-1 text-sm text-slate-500">Risk: {crimeReport.live_level} · score {crimeReport.risk_score}/100 · estimated total {crimeReport.estimated_total}</p></div><button onClick={() => setCrimeReport(null)} className="text-sm font-bold text-slate-500">Close</button></div><div className="mt-6 grid gap-3">{crimeReport.categories.map((item) => <div key={item.category} className="grid grid-cols-[minmax(0,1fr)_4rem] items-center gap-4"><div><div className="flex justify-between gap-3 text-sm font-bold text-slate-700"><span>{item.category}</span><span>{item.estimated_zone_count}</span></div><div className="mt-1 h-2 overflow-hidden rounded bg-slate-100"><div className="h-full bg-rose-500" style={{ width: `${Math.max(2, item.estimated_zone_count / crimeReport.estimated_total * 100)}%` }} /></div></div><span className="text-right text-[10px] font-bold text-slate-400">district {item.district_count}</span></div>)}</div><div className="mt-7 rounded-2xl bg-amber-50 p-4 text-sm text-amber-950"><strong>Authority briefing:</strong> verify the situation using police/field inputs before deployment. For an emergency, use the existing SOS process and local emergency contacts; this is a prioritisation signal only.</div></div>}
                            </div>
                        )}
                         {activeTab === 'emergency' && (
